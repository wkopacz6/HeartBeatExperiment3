...
expName = 'HeartBeat2_2'  # from the Builder filename that created this script
...
filename = _thisDir + os.sep + u'Heartdata1119/%s_%s_%s' % (expInfo['participant'], expName, expInfo['date'])

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='/Users/walterkopacz/anaconda2/envs/psychopy/conda-meta/HeartBeat2_2.py',
...
Question = visual.TextStim(win=win, name='Question',
    text='Using the keypad, rate on a scale of 1 to 5 how strongly you felt what this person felt.',
    font='Arial',
    pos=(0, .2), height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    depth=0.0);
scale = visual.ImageStim(
    win=win, name='scale',units='pix', 
    image='scale2.png', mask=None,
    ori=0, pos=(0, -200), size=(1162, 158),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('Heart2Cond.xlsx'),
...
for thisTrial in trials:
    ...
    outlet.push_sample(x=['Start Experiment'])
    
    
    while continueRoutine and routineTimer.getTime() > 0:
        img = 'Start: ' + pic
        conditions = 'Condition: ' + marker
        outlet.push_sample(x=[img])
        outlet.push_sample(x=[conditions])
        # get current time
    ...
    continueRoutine = True
    # update component parameters for each repeat
    EmpAnswer = event.BuilderKeyResponse()
    # keep track of which components have finished
    EmpRateComponents = [Question, EmpAnswer, scale]
    for thisComponent in EmpRateComponents:
        if hasattr(thisComponent, 'status'):
            thisComponent.status = NOT_STARTED
    
    # -------Start Routine "EmpRate"-------
    outlet.push_sample(x=['End Stimuli'])
    outlet.push_sample(x=['Correct Answer: ' + answer])
    outlet.push_sample(x=['Part. Answer: ' + choose])
    while continueRoutine:
        # get current time
        ...
        # *EmpAnswer* updates
        if t >= 0.0 and EmpAnswer.status == NOT_STARTED:
            # keep track of start time/frame for later
            EmpAnswer.tStart = t
            EmpAnswer.frameNStart = frameN  # exact frame index
            EmpAnswer.status = STARTED
            # keyboard checking is just starting
            win.callOnFlip(EmpAnswer.clock.reset)  # t=0 on next screen flip
            event.clearEvents(eventType='keyboard')
        if EmpAnswer.status == STARTED:
            ...
            if "escape" in theseKeys:
                endExpNow = True
            if len(theseKeys) > 0:  # at least one key was pressed
                EmpAnswer.keys = theseKeys[-1]  # just the last key pressed
                EmpAnswer.rt = EmpAnswer.clock.getTime()
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in EmpRateComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # check responses
    if EmpAnswer.keys in ['', [], None]:  # No response was made
        EmpAnswer.keys=None
    trials.addData('EmpAnswer.keys',EmpAnswer.keys)
    if EmpAnswer.keys != None:  # we had a response
        trials.addData('EmpAnswer.rt', EmpAnswer.rt)
    ...
    # -------Start Routine "Wait"-------
    outlet.push_sample(x=['Strength of Feeling: ' + EmpAnswer.keys])
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = WaitClock.getTime()
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
